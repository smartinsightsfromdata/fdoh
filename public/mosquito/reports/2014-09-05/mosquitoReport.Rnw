...
####################
load("C:/Users/BrewJR/Documents/fdoh/public/mosquito/reports/2014-09-05/master.RData")
...
As predicted, mosquito numbers fell over recent weeks to 93 mosquitoes per trap at the most recent trap collection (\Sexpr{format(recent, format="%B %d, %Y")}).  
...
At \Sexpr{round(master$totPer[which(master$date == recent)])} specimens per trap, the most recent collection was lower than our prediction of \Sexpr{round(master$predicted[which(master$date == recent)])}, but remained well within the 90\% confidence range of 0-302.  
