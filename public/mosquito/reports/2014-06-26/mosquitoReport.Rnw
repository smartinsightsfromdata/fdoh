...
\graphicspath{ {E:/fdoh/public/mosquito/reports/2014-06-26} }
...
####################
load("E:/fdoh/public/mosquito/reports/2014-06-26/master.RData")
...
As predicted, the most recent trap collection (\Sexpr{format(recent, format="%B %d, %Y")}) saw medium-low levels of mosquitoes for this time of year, with geographic dispersal throughout the county.  
...
We forecast that mosquito levels will remain low through the weekend, before beginning a steady climb from an equivalent weekly rate of 75 per trap (July 2nd) up to approximately 145 by July 7th (70\% range 0-302).
...
At \Sexpr{master$totPer[which(master$date == max(master$date[which(is.na(master$totPer) == FALSE)]))]} specimens per trap, the most recent collection was very similar to our prediction of \Sexpr{round(master$predicted[which(master$date == max(master$date[which(is.na(master$totPer) == FALSE)]))], digits=0)} specimens per trap (and well within our 70\% confidence interval of 0-277).
...
Of recently trapped mosquitoes, those species capable of carrying St. Louis Encephalitis saw a rise last week.
...
hist(bootPred,
     ...
     cex.lab=0.75,
     cex.axis=0.8)
title(main="Prediction for period through July 7, 2014",
      cex.main=0.75)
axis(side=1,
