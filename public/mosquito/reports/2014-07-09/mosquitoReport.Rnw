...
\graphicspath{ {E:/fdoh/public/mosquito/reports/2014-07-09} }
...
####################
load("E:/fdoh/public/mosquito/reports/2014-07-09/master.RData")
...
As predicted, the most recent trap collection (\Sexpr{format(recent, format="%B %d, %Y")}) saw rising levels of mosquitoes, with geographic dispersal throughout the county.  
...
At \Sexpr{master$totPer[which(master$date == max(master$date[which(is.na(master$totPer) == FALSE)]))]} specimens per trap, the most recent collection was higher than our prediction of \Sexpr{round(master$predicted[which(master$date == max(master$date[which(is.na(master$totPer) == FALSE)]))], digits=0)}.  
