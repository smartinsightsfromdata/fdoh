...
\graphicspath{ {E:/fdoh/public/mosquito/reports/2014-08-01} }
...
####################
load("E:/fdoh/public/mosquito/reports/2014-08-01/master.RData")
...
As predicted, mosquito numbers rose steadily for two weeks before declining slightly the most recent trap collection (\Sexpr{format(recent, format="%B %d, %Y")}) saw moderate levels (125 mosquitoes per trap).  
...
At \Sexpr{master$totPer[which(master$date == max(master$date[which(is.na(master$totPer) == FALSE)]))]} specimens per trap, the most recent collection was slightly higher than our prediction of \Sexpr{round(master$predicted[which(master$date == max(master$date[which(is.na(master$totPer) == FALSE)]))], digits=0)}, but within the 90\% confidence range of 0-224.  
