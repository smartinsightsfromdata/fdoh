...
####################
load("E:/workingdirectory/mosquito/reports/2014-06-06/master.RData")
...
At \Sexpr{master$totPer[which(master$date == max(master$date[which(is.na(master$totPer) == FALSE)]))]} specimens per trap, the most recent trap collection (\Sexpr{format(recent, format="%B %d, %Y")}) saw normal (medium) levels of mosquitoes for this time of year, with a concentration in the southeastern area of the county.  
...
hist(bootPred,
     ...
     cex.lab=0.75,
     cex.axis=0.8)
title(main="Prediction for period through June 16, 2014",
      cex.main=0.75)
axis(side=1,
