...
####################
load("E:/fdoh/public/mosquito/reports/2014-08-21/master.RData")
...
As predicted, mosquito numbers remained largely steady over recent weeks, peaking slightly at the most recent trap collection (\Sexpr{format(recent, format="%B %d, %Y")}) (242 mosquitoes per trap).  
...
At \Sexpr{round(master$totPer[which(master$date == max(master$date[which(is.na(master$totPer) == FALSE)]))])} specimens per trap, the most recent collection was significantly higher than our prediction of \Sexpr{round(master$predicted[which(master$date == max(master$date[which(is.na(master$totPer) == FALSE)]))], digits=0)}, but remained (barely) within the 90\% confidence range of 0-242.  
...
hist(bootPred,
     ...
     cex.lab=0.75,
     cex.axis=0.8)
title(main="Prediction for period through August 26, 2014",
      cex.main=0.75)
axis(side=1,
